# 🧠 База знаний разработчика

Добро пожаловать в мой личный «архив» знаний. Здесь я собираю полезные ресурсы, шпаргалки и собственные наработки, чтобы всегда иметь под рукой структурированную информацию.

> **Содержание:**
> 1.  Полезные сайты
> 2.  Полезные репозитории и GitHub-аккаунты
> 3.  Мои решения задач

---

## 🌐 Полезные сайты

### 🎓 Образовательные платформы и курсы
| Платформа | Описание |
| :--- | :--- |
| [Stepik](https://stepik.org/learn) | Классическая платформа с огромным выбором курсов от ведущих ВУЗов и компаний. |
| [Яндекс Лицей](https://lyceum.yandex.ru/#selfpaced_courses) | Бесплатные курсы по Python, Go и анализу данных от экспертов Яндекса. |
| [Яндекс Практикум](https://practicum.yandex.ru/profile/python-free/) | Бесплатный вводный курс по Python с тренажерами. |
| [Coddy](https://coddy.tech/journeys/python/logic_and_flow) | Интерактивное обучение программированию с игровым уклоном. |
| [VK Education](https://education.vk.company) | Курсы и лекции от VK по разработке, дизайну и управлению продуктами. |

### ⚔️ Тренировка алгоритмов (Coding Challenge)
| Платформа | Описание |
| :--- | :--- |
| [Coderun](https://coderun.yandex.ru) | Удобная площадка от Яндекса для отработки алгоритмических задач. |
| [Codewars](https://www.codewars.com/dashboard) | Тренировка в стиле «боевых искусств» — повышайте свой ранг, решая задачи. |
| [Leetcode](https://leetcode.com/problemset/) | Наиболее популярный ресурс для подготовки к собеседованиям в IT-гиганты. |
| [Тренировки от Яндекса](https://yandex.ru/yaintern/training/index) | Обучение от Яндекса по различным направлениям |

### 📚 Справочники и документация
| Ресурс | Описание |
| :--- | :--- |
| [Shadowhint](https://shadowhint.com/questions?technologies=826ac86f-b7a5-43db-9e73-ab5cfbad431a) | База знаний по языкам программирования с ответами на частые вопросы. |
| [Metanit](https://metanit.com) | Классический русскоязычный сайт с исчерпывающей документацией по C#, Java, Python и другим языкам. |

### 🛠️ Дополнительные инструменты
| Ресурс | Описание |
| :--- | :--- |
| [Learn Git Branching](https://learngitbranching.js.org/?locale=ru_RU) | Интерактивный тренажер по Git. Наглядно показывает, как работают ветки и слияния. |
| [Полезные материалы Yandex](https://yandex.ru/yaintern/schools/useful) | Подборка лекций и статей для начинающих разработчиков. |
| [Loecsen](https://www.loecsen.com/ru/kурсы-английский) | Изучение иностранных языков с акцентом на бытовую лексику. |

---

## 📂 Полезные репозитории и GitHub-аккаунты

### Интересные репозитории
| Репозиторий | Назначение |
| :--- | :--- |
| [Project-Based Learning](https://github.com/practical-tutorials/project-based-learning?tab=readme-ov-file#python) | Огромный список пет-проектов на разных языках. Идеи для портфолио. |
| [30 Days Of Python](https://github.com/Asabeneh/30-Days-Of-Python) | 30-дневный челлендж по Python с нуля до продвинутого уровня. |
| [Developer Roadmap](https://github.com/kamranahmedse/developer-roadmap) | Визуальные карты развития для frontend, backend, DevOps и других направлений. |
| [Markdown Cheatsheet](https://gist.github.com/lorddead12/f2214490c9404683c0ee9eb9d3a6e389) | Моя шпаргалка по оформлению `.md` файлов. |

---

## 🧩 Мои решения задач

Здесь я храню код, который написал в процессе обучения на различных платформах.

| Платформа | Ссылка на решения |
| :--- | :--- |
| **Stepik** | [Перейти к решениям →](https://github.com/Hohrandrey/self-study/tree/main/Stepik) |
| **Codewars** | [Перейти к решениям →](https://github.com/Hohrandrey/self-study/tree/main/Codewars) |

---

**📌 Важно:** Репозиторий постоянно обновляется, следите за обновлениями!
