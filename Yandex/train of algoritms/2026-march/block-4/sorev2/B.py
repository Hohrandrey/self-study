...
for i in range(m):
    for j in range(i + 1, m):
        ...
        correct_matches_i_to_j = sum(1 for k in correct_indices_i if ans_j[k] == ans_i[k])
        wrong_matches_i_to_j = sum(1 for k in wrong_indices_i if ans_j[k] == ans_i[k])

        if (correct_matches_i_to_j > len(correct_indices_i) / 2 and
            wrong_matches_i_to_j > len(wrong_indices_i) / 2 and
            correct_matches_i_to_j > len(correct_indices_j) / 2 and
            wrong_matches_i_to_j > len(wrong_indices_j) / 2):
